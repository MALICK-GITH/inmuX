...
# Real ANSI escape sequences (not literal \\033 text).
if [[ -t 1 && -t 0 ]]; then
...
install_deps() {
  ...
  pkg update -y
  pkg install -y bash curl dnsutils whois nmap grep sed coreutils findutils
...
EOF
    ...
    case "$choice" in
      1) dns_lookup;; 2) reverse_dns;; 3) whois_lookup;; 4) geoip_lookup;; 5) host_finder;;
      6) http_headers;; 7) host_dns;; 8) port_scan;; 9) subnet_lookup;; 10) zone_transfer;; 11) extract_links;;
      12) view_reports;; 13) delete_report;; 14) delete_all;; 97) install_deps;; 98) about;; 00|0|q|Q) break;;
      *) printf '%s[!] Unknown option. Choose an item from the menu.%s\n' "$YELLOW" "$RESET"; sleep 1;;
    esac
  done
  printf '%s[✓] Goodbye.%s\n' "$GREEN" "$RESET"
}
