...
# Colors
if [[ -t 1 ]]; then
  GREEN='\033[32;1m'; YELLOW='\033[33;1m'; BLUE='\033[34;1m'
  RED='\033[31;1m'; CYAN='\033[36;1m'; MAGENTA='\033[35;1m'; RESET='\033[0m'
else
...
banner() {
  clear
  printf '%s╔══════════════════════════════════════════════╗%s\n' "$RED" "$RESET"
  printf '%s║        SOLITAIRE HACK — inmuX v%s         ║%s\n' "$RED" "$VERSION" "$RESET"
...
ask_target() {
  ...
  safe_target "$target" || { printf '%s[!] Invalid target.%s\n' "$RED" "$RESET"; return 1; }
  TARGET="$target"
}
...
geoip_lookup() {
  ask_target || return
  new_report geoip
  run_report 'GeoIP metadata' bash -c 'header "GeoIP"; curl -fsSL --max-time "$2" "https://ipwho.is/$1"' _ "$TARGET" "$TIMEOUT"
  finish
}
...
host_finder() {
  ...
  run_report 'Subdomain/host discovery' bash -c 'header "Host discovery"; curl -fsSL --max-time "$2" "https://api.hackertarget.com/hostsearch/?q=$1"' _ "$TARGET" "$TIMEOUT"
  finish
}
...
http_headers() {
  ...
  run_report 'HTTP security headers' bash -c 'header "HTTP headers"; curl -sSIL --max-time "$2" --connect-timeout 7 "$1"' _ "$url" "$TIMEOUT"
  finish
}
...
subnet_lookup() {
  ask_target || return
  new_report subnet
  run_report 'Subnet calculation' bash -c 'header "Subnet"; curl -fsSL --max-time "$2" "https://api.hackertarget.com/subnetcalc/?q=$1"' _ "$TARGET" "$TIMEOUT"
  finish
}
...
zone_transfer() {
  ...
  read -r -p 'Continue? (y/N): ' confirm
  [[ "$confirm" =~ ^[Yy]$ ]] || return
  run_report 'DNS zone transfer check' bash -c 'header "AXFR"; servers=$(dig +short NS "$1"); for ns in $servers; do echo "== $ns =="; dig +time=3 +tries=1 AXFR "$1" "@$ns"; done' _ "$TARGET"
  finish
}
...
extract_links() {
  ...
  local url="$TARGET"
  [[ "$url" == http://* || "$url" == https://* ]] || url="https://$url"
  run_report 'Extracting links' bash -c 'header "Links"; curl -fsSL --max-time "$2" "$1" | grep -Eoi "https?://[^\"[:space:]<>]+" | sed "s/[),.;]$//" | sort -u' _ "$url" "$TIMEOUT"
  finish
}
...
view_reports() {
  ...
  ((${#files[@]})) || { printf '%sNo reports yet.%s\n' "$YELLOW" "$RESET"; read -r -p 'Press Enter...' _; return; }
  ...
  [[ "$choice" =~ ^[0-9]+$ ]] || return
  ((choice >= 1 && choice <= ${#files[@]})) || { printf '%sInvalid choice.%s\n' "$RED" "$RESET"; sleep 1; return; }
...
delete_report() {
  ...
  mkdir -p "$OUTDIR"
  mapfile -t files < <(find "$OUTDIR" -maxdepth 1 -type f -printf '%f\n' | sort -r)
  ((${#files[@]})) || { printf '%sNo reports.%s\n' "$YELLOW" "$RESET"; sleep 1; return; }
  local i=1 f
  for f in "${files[@]}"; do printf '%3d  %s\n' "$i" "$f"; ((i++)); done
...
EOF
    ...
    case "$choice" in
      ...
      9) subnet_lookup;; 10) zone_transfer;; 11) extract_links;;
      12) view_reports;; 13) delete_report;; 14) delete_all;;
      97) install_deps;; 98) about;; 00) printf 'À bientôt !\n'; exit 0;;
